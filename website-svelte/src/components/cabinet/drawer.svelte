...
<script lang="ts" context="module">
    ...
    import { useTypedContext } from "~/utils/useTypedContext";

    const {
        get: usePreventDrawerClose,
        set: setPreventDrawerClose,
    } = useTypedContext("prevent_drawer_close");
    export { usePreventDrawerClose };
</script>
...
<script lang="ts">
    ...
    const prevent_drawer_close = writable(0);
    setPreventDrawerClose(prevent_drawer_close);
    const prevent_drawer_open = usePreventDrawerOpen();

    const dont_open = $prevent_drawer_open !== -1 && $prevent_drawer_open !== index;

    $: {
        if ($prevent_drawer_open === -1 && $prevent_drawer_close > 0) {
            $prevent_drawer_open = index;
        }
        else if ($prevent_drawer_open === index && $prevent_drawer_close == 0) {
            $prevent_drawer_open = -1;
        }
    };
</script>
...
<button
    tabIndex={index}
    class="drawer"
    class:prevent-close={$prevent_drawer_close > 0}
    class:prevent-open={dont_open}
>
    <div class="background">
        {#each { length: 13 } as _a, _b}
            <Paper />
        {/each}
    </div>
